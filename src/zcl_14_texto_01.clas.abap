...
CLASS zcl_14_texto_01 IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    out->write( |FROM_MIXED  =  { from_mixed( lv_string2 ) } | ).

    out->write( |  | ).

    "ORDER
    out->write( |REVERSE = { reverse( lv_string2 ) } | ).
    out->write( |SHIFT_LEFT (places)   = { shift_left(   val = lv_string2 places = 5 ) } | ).
    out->write( |SHIFT_RIGHT (places)  = { SHIFT_right(  val = lv_string2 places = 5 ) } | ).
    out->write( |SHIFT_LEFT (places)   = { shift_left(   val = lv_string2 circular = 5 ) } | ).
    out->write( |SHIFT_RIGHT (places)  = { shift_RIGHT(  val = lv_string2 circular = 5 ) } | ).

    out->write( |  | ).
    "Substring
    out->write( |SUBSTRING          = { substring(         val = lv_string2 off = 9 len = 6 ) } | ).
    out->write( |SUBSTRING_FROM     = { substring_from(    val = lv_string2 sub = 'ABAP' ) } | ).
    out->write( |SUBSTRING_AFTER    = { substring_after(   val = lv_string2 sub = 'ABAP' ) } | ).
    out->write( |SUBSTRING_TO       = { substring_to(      val = lv_string2 sub = 'ABAP' ) } | ).
    out->write( |SUBSTRING_BEFORE   = { substring_before(  val = lv_string2 sub = 'ABAP' ) } | ).

    out->write( |  | ).
    "OTHERS
    out->write( |CONDENSE   = { condense(  val = lv_string2 )  } | ).
    out->write( |REPEAT     = { repeat(    val = lv_string2 occ = 2 )  } | ).
    out->write( |SEGMENT1   = { segment(   val = lv_string2 sep = '!' index = 1 )  } | ).
    out->write( |SEGMENT2   = { segment(   val = lv_string2 sep = '!' index = 2 )  } | ).
